...
const TestCaseBuilder: React.FC<TestStepProps> = ({ param }) => {
  ...
  const handleStepChange = (stepNumber: number, field: keyof TestStep, value: any, client:boolean) => {
    setisclient(client)
    setTestCase(prev => ({
      ...prev,
      testcasedetails: {
            ...
            <div key={index} className="border p-4 rounded-lg relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <select
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.target || ''}
                    onChange={(e) => handleStepChange(index + 1, 'target', e.target.value as TestTarget,e.target.value !== "DUT" ? true : false)}
                  >
                ...
                <div>
                  ...
                  <select
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.method || ''}
                    onChange={(e) => handleStepChange(index + 1, 'method', e.target.value as TestMethod,false)}
                  >
                ...
                <div className="md:col-span-2">
                  ...
                  <Input
                    type="text"
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.param || ''}
                    onChange={(e) => handleStepChange(index + 1, 'param', e.target.value,false)}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    type="text"
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.expectedResult || ''}
                    onChange={(e) => handleStepChange(index + 1, 'expectedResult', e.target.value,false)}
                  />
                </div>

                <div>
                  <Label className="block text-sm font-medium mb-1">Post Wait (seconds)</Label>
                  <Input
                    type="number"
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.postWaitInSecs || ''}
                    onChange={(e) => handleStepChange(index + 1, 'postWaitInSecs', parseInt(e.target.value),false)}
                  />
                </div>

                <div>
                  <Label className="block text-sm font-medium mb-1">Retry on Fail</Label>
                  <Input
                    type="number"
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.retryOnFail || ''}
                    onChange={(e) => handleStepChange(index + 1, 'retryOnFail', parseInt(e.target.value),false)}
                  />
                </div>

                <div>
                  <Label className="block text-sm font-medium mb-1">Break on Failure</Label>
                  <select
                    className="w-full p-2 border rounded"
                    value={testCase.testcasedetails[`step${index + 1}`]?.breakOnFailure?.toString() || 'false'}
                    onChange={(e) => handleStepChange(index + 1, 'breakOnFailure', e.target.value,false)}
                  >
